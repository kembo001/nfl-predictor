"""
NFL Playoff Model v16.1 — v16 + fixes for tossups, leakage-free rolling backtest, and baseline-only ridge

KEY FIXES vs v16 (your paste):
1) FIXED toss-up logic for baseline games:
   - Previously, missing spread defaulted to abs_spread=7.0, which made baseline tossups almost impossible.
   - Now: baseline games can be tossups on seeds alone; we also add `tossup_baseline`.

2) RIDGE TRAINING now baseline-only (no train/inference mismatch):
   - v16 trained ridge on spread+baseline, but inference used λ_spread=0 (discarding ridge adj on spread games).
   - Now: ridge learns ONLY baseline adjustments. Spread games return the spread-offset probability directly.

3) Rolling validation is now leakage-free:
   - v16 reused global `best_alpha` (picked with 2023) and global `xgb_weight` for ALL earlier years.
   - Now: each window tunes ridge alpha (on calib years) and optionally tunes XGB weight (on calib years) using only past data.

4) More conservative XGBoost defaults for tiny baseline sample sizes.
...
# ============================================================
# FEATURE ENGINEERING (v15.1 + v16.1 fixes)
# ============================================================
...
def create_spread_features(home_spread):
    ...
    v16.1: missing spread is now explicitly flagged via `has_spread`,
    and confidence is 0.0 when missing (not 0.5).
    """
...
def create_tossup_features(seed_diff, home_spread):
    """
    v16.1 FIX:
    - Baseline games (no spread) should not default to "medium spread".
    - Toss-up now triggers on seeds alone when spread is missing (uncertainty is high).
    """
    ...
    if has_spread:
        ...
        close_spread = True  # missing spread => treat as uncertain/close rather than "7.0"
...
# ============================================================
# SPREAD MODEL (unchanged from your v16)
# ============================================================
...
# ============================================================
# RIDGE MODEL (v16.1: baseline-only)
# ============================================================

def train_ridge_offset_model(features_df, feature_cols, train_seasons, alpha=1.0):
    """
    v16.1:
    Train ridge ONLY on baseline games. This prevents the "train on spread games, but ignore in inference" mismatch.
    """
    train_df = features_df[
...
# ============================================================
# CALIBRATED XGBOOST (more conservative defaults)
# ============================================================
...
def train_xgboost_calibrated(features_df, feature_cols, train_seasons, calib_seasons,
    ...
    """
    v16.1:
    - More conservative XGB defaults for small sample sizes.
    - Still uses Platt scaling on calib_seasons.
    """
    if not HAS_XGBOOST:
        return None
...
# ============================================================
# DYNAMIC ENSEMBLE WEIGHT TUNING
# ============================================================
...
# ============================================================
# LAMBDA TUNING (keep λ_spread=0.0)
# ============================================================
...
def predict_ensemble_v16(model_dict, game_features, platt_params, lambda_params,
    ...
    - Baseline games: ridge + calibrated XGB ensemble.
    """
    src = game_features["offset_source"]

    # Spread games: trust spread-offset (market mapping) directly
    if src == "spread":
...
# ============================================================
# 2024 VALIDATION (unchanged except it works with spread bypass)
# ============================================================
...
def validate_on_2024(model_dict, features_df, platt_params, lambda_params,
    ...
    print(f"2024 PLAYOFF VALIDATION - MODEL v16.1 (λ_spread={lambda_params['spread']:.1f}, λ_base={lambda_params['base']:.1f}{ensemble_str})")
    print("=" * 140)
    ...
    for _, gf in test_df.iterrows():
        ...
        row_data = {
            ...
            "lambda_used": float(pred.get("lambda_used", 0.0)),
            "predicted": pred_team,
            "actual": actual,
    ...
    print(f"  Model v16.1:  {m['log_loss']:.4f}")
    ...
    if len(tossup_games) > 0:
        print(f"\nToss-up Games (seed_diff ≤ 1, close spread or missing spread):")
...
def main():
    print("=" * 100)
    print("NFL PLAYOFF MODEL v16.1 - BASELINE-ONLY RIDGE + CALIBRATED XGBOOST ENSEMBLE")
    print("=" * 100)
    ...
    # Prepare features
    print("\nPreparing v16.1 features (fixed toss-ups + has_spread flag)...")
    ...
    # Feature columns (baseline model uses these; spread games are bypassed at inference)
    feature_cols = [
        # Base
        ...
        # Round
        "is_superbowl",
        # Spread-related flags (safe; baseline games will have zeros)
    ...
    # Tune ridge alpha on 2023 baseline games ONLY (consistent with baseline-only training)
    print("\nTuning ridge alpha on 2023 (baseline games only)...")
    ...
    # Train final baseline-only ridge
    ...
    # Train calibrated XGBoost for baseline games
    xgb_model = None
    xgb_model_full = None
    if HAS_XGBOOST:
    ...
    # Tune XGB ensemble weight (baseline only)
    xgb_weight = 0.3
    ...
    # ============================================================
    # HISTORICAL ROLLING VALIDATION (leakage-free)
    # ============================================================
    print("\n" + "=" * 140)
    print("HISTORICAL ROLLING VALIDATION (2015-2024) - v16.1 (LEAKAGE-FREE)")
    print("=" * 140)
    ...
    for test_year in range(2015, 2025):
        ...
        # Tune ridge alpha on calib years (baseline only) — leakage-free
        best_a, best_ll = None, float("inf")
        for a in alpha_grid:
            mtmp = train_ridge_offset_model(w_features, avail_cols, train_yrs, alpha=a)
            if mtmp is None:
                continue

            val_df = w_features[
                (w_features["season"].isin(calib_yrs)) & (w_features["offset_source"] == "baseline")
            ].dropna(subset=avail_cols + ["offset_logit"])

            preds = []
            for _, r in val_df.iterrows():
                pr = predict_raw(mtmp, r)
                if pr is None:
                    continue
                p = pr["raw_prob"]
                actual = int(r["home_wins"])
        ...
        model = train_ridge_offset_model(w_features, avail_cols, train_yrs, alpha=best_a)
        if model is None:
            continue

        # Train calibrated XGB for baseline games in this window
        w_xgb = None
        if HAS_XGBOOST:
            w_baseline_df = w_features[w_features["offset_source"] == "baseline"]
            w_xgb = train_xgboost_calibrated(w_baseline_df, avail_cols, train_yrs, xgb_calib_yrs)
        ...
        # Tune ensemble weight on calib years (baseline only) — leakage-free
        w_xgb_weight = 0.0
        if w_xgb is not None:
            w_xgb_weight = tune_ensemble_weight(
                model, w_xgb, w_features,
                val_seasons=calib_yrs,
                lambda_base=lam_params["base"]
            )
        ...
        if results:
            ...
            all_results.append({
                ...
                "acc": float(rdf["correct"].mean()),
                "model_ll": model_ll,
                "offset_ll": offset_ll,
                "base_ll": base_ll,
                "lam_spread": float(lam_params["spread"]),
                "lam_base": float(lam_params["base"]),
                "alpha": float(best_a) if best_a is not None else np.nan,
                "xgb_w": float(w_xgb_weight),
            })

    hist_df = pd.DataFrame(all_results)

    print(f"\n{'Season':<8} {'N':<5} {'Acc':<7} {'Model':<8} {'Offset':<8} {'Base':<8} {'vs Base':<9} {'vs Off':<9} {'α':<4} {'w':<4}")
    print("-" * 110)
    ...
    # Save
    out_path = "../validation_results_2024_v16_1.csv"
    ...
    print(f"""
    FIXED IN v16.1:
    ✓ Baseline toss-up detection works (no more abs_spread=7.0 default)
    ✓ Added has_spread + tossup_baseline flags
    ✓ Ridge trained baseline-only (no spread train/inference mismatch)
    ✓ Leakage-free rolling backtest (tune α and XGB weight per window)
    ✓ More conservative XGBoost defaults for small baseline sample sizes

    STRATEGY:
    ✓ Spread games: use spread-offset probability directly
