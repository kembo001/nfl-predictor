...
def load_training_data():
    games_df = pd.read_csv(BASE_DIR / ".." / "nfl_playoff_results_2000_2024_with_epa.csv")
    team_df = pd.read_csv(BASE_DIR / ".." / "df_with_upsets_merged.csv")
    spread_df = pd.read_csv(BASE_DIR / ".." / "csv" / "nfl_biggest_playoff_upsets_2000_2024.csv")
...
def load_wc_lines_2025_inline():
    ...
    df["away_team"] = df["matchup"].str.split(" @ ").str[0].str.strip().str.upper()
    df["home_team"] = df["matchup"].str.split(" @ ").str[1].str.strip().str.upper()
...
def fit_spread_model(features_df, train_seasons, baselines, alpha=0.5):
    """Fit spread-to-probability model.
    
    FIXED: Intercept now anchored toward 0 (not home_win_rate) because
    point spreads already include home field advantage.
    spread=0 should imply ~50%, not historical home win rate.
    
    NOTE: Excludes neutral-site games (Super Bowl) since they don't have
    home field advantage baked into the spread.
    """
    train_df = features_df[
        (features_df["season"].isin(train_seasons))
        & (features_df["home_spread"].notna())
        & (features_df["is_neutral"] == 0)  # avoid SB/neutral contamination
    ].copy()
    if len(train_df) < 20: return None
    
    # Fit directly on spread points (not divided by 7)
    ...
    prior_slope = -0.073  # ~3% per point is typical market conversion
    w = len(train_df) / (len(train_df) + 60)
    
    slope = float(np.clip(model.params[1], -0.12, -0.04))
    slope_final = w * slope + (1 - w) * prior_slope
    
    # FIXED: Hard force intercept to 0 (50% at spread=0)
    # Point spreads ALREADY include home field advantage
    intercept_final = 0.0
...
def sanity_check_spread_model(spread_model):
    """Print spread→prob conversions to verify the model is centered correctly."""
    if spread_model is None:
        print("  No spread model to check.")
        return
    print("\n  Spread → Home Win Prob sanity check:")
    print("  (spread=0 should be ~50%, negative=home favored, positive=home underdog)")
    for s in [0, -1, -3.5, -7, +1, +3.5, +7, +10.5]:
        p = expit(spread_model["intercept"] + spread_model["slope_per_point"] * s)
        label = "home favored" if s < 0 else "home underdog" if s > 0 else "pick'em"
        print(f"    spread={s:+5.1f} ({label:12s}) → {p:.1%}")
...
def prepare_game_features(games_df, team_df, epa_model, baselines, spread_model):
    """Prepare features for historical games. Fixed: neutral-site swap now also swaps team names."""
    ...
    for _, game in games_df.iterrows():
        ...
        # Local copies so we can safely swap
        away_team = game["away_team"]
        home_team = game["home_team"]
        winner = game["winner"]
        home_spread = game.get("home_spread", np.nan)

        away_row = team_df[(team_df["team"] == away_team) & (team_df["season"] == season)]
        home_row = team_df[(team_df["team"] == home_team) & (team_df["season"] == season)]
        ...
        # Neutral logic: force "home" to be the better seed for consistent baseline logic
        ...
        row = {
            ...
            "home_team": home_team,
            "winner": winner,
            "home_wins": 1 if winner == home_team else 0,  # IMPORTANT: uses swapped home_team
            ...
            "delta_pd_pg": float(home_data["point_differential"] / max(home_games, 1)
                                 - away_data["point_differential"] / max(away_games, 1)),
            "delta_vulnerability": (
                (float(away_data["win_pct"]) - float(epa_model["intercept"] + epa_model["slope"] * away_net_epa))
            ...
            "delta_momentum": float(home_data.get("momentum_residual", 0) or 0)
                              - float(away_data.get("momentum_residual", 0) or 0),
...
# ============================================================
# MATCHUP PRICER (Fast hypothetical matchup pricing)
# ============================================================

class MatchupPricer:
    """
    Fast pricer for hypothetical matchups.
    - Precomputes z-scores once
    - Precomputes quality ranks once per season
    - Caches feature rows + predictions
    """
    ...
    def __init__(
        ...
        self.adj_cap = float(adj_cap)
        self.tossup_shrink = float(tossup_shrink)

        # Precompute z-scores ONCE
        self.team_df_z = compute_season_zscores(team_df)

        # Slice to season and build quick lookup
        season_df = self.team_df_z[self.team_df_z["season"] == self.season].copy()
        self.team_row = {r["team"]: r for _, r in season_df.iterrows()}

        # Precompute qrank map ONCE
        season_df["pd_pg"] = season_df["point_differential"] / (
            (season_df["wins"] + season_df["losses"]).clip(lower=1)
        )
        season_df["qrank"] = season_df["pd_pg"].rank(ascending=False)
        self.qrank = {r["team"]: int(r["qrank"]) for _, r in season_df.iterrows()}

        # Caches
        self._feat_cache = {}
        self._pred_cache = {}

    def _build_features(self, away_team, home_team, game_type, neutral=False, home_spread=np.nan):
        key = (game_type, away_team, home_team, bool(neutral), None if pd.isna(home_spread) else float(home_spread))
        ...
        if away_team not in self.team_row or home_team not in self.team_row:
            return None

        away_data = self.team_row[away_team]
        home_data = self.team_row[home_team]
        ...
        # Neutral: force better seed to be "home" for consistent baseline logic
        if is_neutral and away_seed < home_seed:
            away_team, home_team = home_team, away_team
            away_data, home_data = home_data, away_data
            away_seed, home_seed = home_seed, away_seed
        ...
        away_exp = float(self.epa_model["intercept"] + self.epa_model["slope"] * away_net_epa)
        home_exp = float(self.epa_model["intercept"] + self.epa_model["slope"] * home_net_epa)

        away_win_pct = float(away_data["win_pct"])
        home_win_pct = float(home_data["win_pct"])

        away_q = self.qrank.get(away_team, len(self.qrank) // 2)
        home_q = self.qrank.get(home_team, len(self.qrank) // 2)

        seed_diff = away_seed - home_seed
        baseline_prob = float(baseline_probability(home_seed, away_seed, is_neutral, self.baselines))
        baseline_logit = float(logit(np.clip(baseline_prob, 0.01, 0.99)))

        spread_offset = get_spread_offset_logit(home_spread, self.spread_model)
        if pd.notna(spread_offset):
            offset_logit = float(spread_offset)
            offset_source = "spread"
        else:
        ...
        row = {
            ...
            # return both (current orientation + original request)
            "away_team": away_team,
            "home_team": home_team,
            "orig_away_team": orig_away,
            "orig_home_team": orig_home,
            ...
            "delta_pd_pg": home_pd_pg - away_pd_pg,
            "delta_vulnerability": (away_win_pct - away_exp) - (home_win_pct - home_exp),
            "delta_underseeded": (away_seed - away_q) - (home_seed - home_q),
            "delta_momentum": float(home_data.get("momentum_residual", 0) or 0)
                              - float(away_data.get("momentum_residual", 0) or 0),
        ...
        self._feat_cache[key] = row
        return row

    def price(self, away_team, home_team, game_type="DIV", neutral=False, home_spread=np.nan):
        """
        Returns a dict with probability + pick.
        If home_spread is provided => uses your spread offset path (market).
        If not => uses baseline + model adjustment.
        """
        key = (game_type, away_team, home_team, bool(neutral), None if pd.isna(home_spread) else float(home_spread))
        if key in self._pred_cache:
            return self._pred_cache[key]

        feat = self._build_features(away_team, home_team, game_type, neutral=neutral, home_spread=home_spread)
        if feat is None:
            out = {"home_prob": 0.5, "pick": None, "error": "Unknown team(s) for season"}
        ...
        pred = predict_ensemble_v16_2(
            ...
            tossup_shrink=self.tossup_shrink,
        )
        if pred is None:
            out = {"home_prob": 0.5, "pick": None, "error": "Prediction failed"}
            self._pred_cache[key] = out
            return out

        away = feat["orig_away_team"]
        home = feat["orig_home_team"]

        # NOTE: pred is in the (possibly swapped) orientation. We want to report
        # in the user-requested orientation (orig_away @ orig_home).
        #
        # If the neutral swap happened, feat["home_team"] may not equal orig_home.
        # We'll map probability back if needed.
        if feat["home_team"] == home:
            home_prob_report = float(pred["home_prob"])
        else:
            # We swapped orientation, invert back
            home_prob_report = float(1.0 - pred["home_prob"])

        pick = home if home_prob_report > 0.5 else away
        out = {
            "away": away,
            "home": home,
            "game_type": game_type,
            "neutral": bool(neutral),
            "home_spread": None if pd.isna(home_spread) else float(home_spread),
            "offset_source": feat["offset_source"],
            "home_prob": home_prob_report,
            "pick": pick,
        }
        self._pred_cache[key] = out
        return out

    def price_from_market_cell(self, away_team, home_team, spread_cell, game_type="WC", neutral=False):
        """Price a matchup using a market spread cell like 'CHI (-1)'."""
        hs = home_spread_from_market(away_team, home_team, spread_cell)
        return self.price(away_team, home_team, game_type=game_type, neutral=neutral, home_spread=hs)
...
def main():
    ...
    # Sanity check: verify spread=0 → ~50% (spread already includes home field advantage)
    sanity_check_spread_model(spread_model)
    ...
    if model_dict is None:
        raise RuntimeError(
            "Ridge model training failed (not enough baseline-only games after filtering). "
            "Consider loosening the baseline-only restriction or ensuring more games have offset_source='baseline'."
        )
    ...
    # Save WC predictions
    wc_out = (BASE_DIR / ".." / "wildcard_predictions_2025_v16_2.csv").resolve()
    ...
    sim_out = (BASE_DIR / ".." / "playoff_sim_2025_v16_2.csv").resolve()
    ...
    # Build WC spread lookup from wc_df
    wc_spread_lookup = {}
    for _, r in wc_df.iterrows():
        wc_spread_lookup[(r["away_team"], r["home_team"])] = r["home_spread"]
    
    # Wild Card Round (using market spreads)
    print("\n" + "-" * 50)
    print("WILD CARD ROUND (with market spreads)")
    print("-" * 50)
    ...
    for away, home, conf in wc_matchups:
        # Use build_single_game_features_row with actual spread
        hs = wc_spread_lookup.get((away, home), np.nan)
        game_feats = build_single_game_features_row(
            2025, "WC", away, home,
            team_df_all, epa_model, baselines, spread_model,
            location="Home", home_spread=hs
        )
    ...
    # Divisional Round (no spreads available)
    print("\n" + "-" * 50)
    print("DIVISIONAL ROUND (baseline offsets)")
    print("-" * 50)
    ...
    # Conference Championships (no spreads available)
    print("\n" + "-" * 50)
    print("CONFERENCE CHAMPIONSHIPS (baseline offsets)")
    print("-" * 50)
    ...
    # Super Bowl (neutral site, no spread)
    print("\n" + "-" * 50)
    print("SUPER BOWL (neutral site, baseline offset)")
    print("-" * 50)
    
    game_feats = build_single_game_features_row(
        2025, "SB", conf_winners["AFC"], conf_winners["NFC"],
        team_df_all, epa_model, baselines, spread_model,
        location="Neutral", home_spread=np.nan
    )
    pred = predict_ensemble_v16_2(model_dict, game_feats, xgb_model, xgb_weight, 1.0, adj_cap, tossup_shrink)
    ...
    print("\n" + "=" * 100)
    print("MATCHUP PRICER DEMO (instant hypothetical pricing)")
    print("=" * 100)
    
    pricer = MatchupPricer(
        team_df=team_df_all,
        season=2025,
        epa_model=epa_model,
        baselines=baselines,
        spread_model=spread_model,
        ridge_model=model_dict,
        xgb_model=xgb_model,
        xgb_weight=xgb_weight,
        adj_cap=adj_cap,
        tossup_shrink=tossup_shrink,
    )
    
    print("\nHypothetical matchups (no spread):")
    print(f"  BUF @ DEN (DIV): {pricer.price('BUF', 'DEN', game_type='DIV')}")
    print(f"  SEA vs DEN (SB neutral): {pricer.price('DEN', 'SEA', game_type='SB', neutral=True)}")
    
    print("\nWith market spread:")
    print(f"  BUF @ JAX with 'BUF (-1.5)': {pricer.price_from_market_cell('BUF', 'JAX', 'BUF (-1.5)', game_type='WC')}")
